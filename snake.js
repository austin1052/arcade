...
function renderGameBoard() {
  ...
  gameBoard.forEach((row, rIdx) => {
    row.forEach((column, cIdx) => {
      if (column === "o") {
        $(".game").append(
          `<div class="snake" style="grid-column:${cIdx}; grid-row:${rIdx + 1
          }"></div>`
        );
      } else if (column === "h") {
        $(".game").append(
          `<div class="head" style="grid-column:${cIdx}; grid-row:${rIdx + 1
          }"></div>`
        );
        currentHead = [rIdx, cIdx];
      } else if (column === "x") {
        $(".game").append(
          `<div class="food" style="grid-column:${cIdx}; grid-row:${rIdx + 1
          }"></div>`
        );
...
function makeFood() {
  let newFoodRow = Math.floor(Math.random() * gameBoardSize);
  let newFoodCol = Math.floor(Math.random() * gameBoardSize) + 1;
  while (gameBoard[newFoodRow][newFoodCol] !== "") {
    newFoodRow = Math.floor(Math.random() * gameBoardSize);
    newFoodCol = Math.floor(Math.random() * gameBoardSize) + 1;
  }
...
function renderHighScore(score, index) {
  const [playerName, playerScore] = score;
